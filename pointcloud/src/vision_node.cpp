...
//waarde voor ROI
#define X_LEFT  -77     // is right (getal hoger = verder naar rechts)    ///// aannpassen
#define X_RIGHT  123 // is left  (getal lager is verder naar rechts)
#define Y_TOP  -73 // is bottom(getal lager is verder naar boven)
#define Y_BOTTOM 170 // is top (getal hoger is naar beneden)
...
void StartCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  if (test == "TRUE") {
    ...
    // Begin Stream with default configs
     ...
     boost::shared_ptr<pcl::visualization::PCLVisualizer> viewerTotal(new pcl::visualization::PCLVisualizer("Total Pointcloud"));
     // Set background of viewer to black
     viewerTotal->setBackgroundColor(0, 0, 0);
     // Add generated point cloud and identify with string "Cloud"
     viewerTotal->addPointCloud<pcl::PointXYZRGB>(cloud, "Picked Object");
     // Default size for rendered points
     viewerTotal->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "Picked Object");
     // Viewer Properties
     viewerTotal->initCameraParameters();  // Camera Parameters for ease of viewing
     //viewer->spin(); // Allow user to rotate point cloud and view it
     viewerTotal->spinOnce();
  ...
  /*===================================================================== zwaartepunt bepalen ======================================================================================*/
     if (clusters.size() == 0) {
      ...
      boost::shared_ptr<pcl::visualization::PCLVisualizer> viewerBakleeg(new pcl::visualization::PCLVisualizer("Empty"));
      // Set background of viewer to black
      viewerBakleeg->setBackgroundColor(0, 0, 0);
      // Add text
      //viewerBakleeg->addText("Bak is leeg", 200, 250, 100, 1.0, 0.0, 0.0);
        viewerBakleeg->addText("Bak is leeg", 100, 400, 50, 1.0, 0.0, 0.0);
        viewerBakleeg->addText("1. Druk op de stop knop", 100, 300, 50, 1.0, 0.0, 0.0);
        viewerBakleeg->addText("2. Vul bak met producten", 100, 200, 50, 1.0, 0.0, 0.0);
        viewerBakleeg->addText("3. Druk op start knop", 100, 100, 50, 1.0, 0.0, 0.0);
      // Viewer Properties
      viewerBakleeg->initCameraParameters();  // Camera Parameters for ease of viewing
      //viewer->spin(); // Allow user to rotate point cloud and view it
      viewerBakleeg->spinOnce();
      ...
      boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(new pcl::visualization::PCLVisualizer("Picked object"));
      // Set background of viewer to black
      viewer->setBackgroundColor(0, 0, 0);
      // Add generated point cloud and identify with string "Cloud"
      viewer->addPointCloud<pcl::PointXYZRGB>(colored_cloud, "Picked Object");
      // Default size for rendered points
      viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "Picked Object");
      // Viewer Properties
